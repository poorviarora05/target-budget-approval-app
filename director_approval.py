...
BUDGET_MASTER_FILE = "budget_master.csv"
...
def load_budget_master():
    try:
        ...
        st.warning(f"Budget master file not found or invalid: {e}")
        return pd.DataFrame()
...
def fetch_dropdown_values(column_key, filters=None):
    df = load_budget_master()
    ...
    column_map = {
        ...
        "job_code": ["job_code"],
        "batch_semester": ["batch_semester", "batch"],
    }

    target_col = get_column_name(df, column_map.get(column_key, [column_key]))
    ...
    if filters:
        for filter_key, filter_value in filters.items():
            filter_col = get_column_name(df, column_map.get(filter_key, [filter_key]))

            if filter_col and filter_value and filter_value != "Not Available":
                filtered_df = filtered_df[
                    filtered_df[filter_col].astype(str).str.strip() == str(filter_value).strip()
                ]
...
def get_budget_values(selected_request):
    total_available_budget = safe_number(selected_request.get("total_available_budget", 0))

    if total_available_budget == 0:
        total_available_budget = safe_number(selected_request.get("available_budget", 0))
...
def show_director_approval():
    ...
    for selected_id in selected_ids:
        ...
        with st.expander(f"Details for Request {selected_id}", expanded=True):
            ...
            business_types = fetch_dropdown_values("business_type")

            with p1:
                business_type = st.selectbox(
                    "Business Type",
                    business_types if business_types else ["B2C", "B2I", "B2B"],
                    key=f"business_type_{selected_id}"
                )

            line_options = fetch_dropdown_values(
                "line_of_business",
                {"business_type": business_type}
            )
            ...
            with p2:
                line_of_business = st.selectbox(
                    ...
                    key=f"line_of_business_{selected_id}"
                )

            program_options = fetch_dropdown_values(
                "programme_name",
                {
                    "business_type": business_type,
                    "line_of_business": line_of_business
            ...
            with p3:
                programme_name = st.selectbox(
                    "Programme Name",
                    program_options if program_options else ["Not Available"],
                    key=f"programme_name_{selected_id}"
                )

            job_options = fetch_dropdown_values(
                "job_code",
                {
                    "business_type": business_type,
                    "line_of_business": line_of_business,
            ...
            with p4:
                job_code = st.selectbox(
                    "Job Code",
                    job_options if job_options else ["Not Available"],
                    key=f"job_code_{selected_id}"
                )

            batch_options = fetch_dropdown_values(
                "batch_semester",
                {
                    "business_type": business_type,
                    "line_of_business": line_of_business,
            ...
            batch_semester = st.selectbox(
                "Batch / Semester",
                batch_options if batch_options else ["Not Available"],
                key=f"batch_semester_{selected_id}"
            )
    ...
    if st.button("Submit Partner Decision"):
        ...
        for selected_id in selected_ids:
            ...
            requests_df.loc[index, "business_type"] = st.session_state.get(
                f"business_type_{selected_id}", ""
            )
            requests_df.loc[index, "line_of_business"] = st.session_state.get(
                f"line_of_business_{selected_id}", ""
            )
            requests_df.loc[index, "programme_name"] = st.session_state.get(
                f"programme_name_{selected_id}", ""
            )
            requests_df.loc[index, "job_code"] = st.session_state.get(
                f"job_code_{selected_id}", ""
            )
            requests_df.loc[index, "batch_semester"] = st.session_state.get(
                f"batch_semester_{selected_id}", ""
            )
