...
def show_mediator_budget_check():
    ...
    st.write("Hours Per Day:", selected_request["hours"])
    st.write("Training Days:", selected_request.get("training_days", 1))
    st.write("Rate Per Hour:", selected_request.get("rate_per_hour", 0))
    st.write("Requester Estimated Budget:", selected_request.get("estimated_budget", 0))
    ...
    st.write("Training Material Required:", selected_request["training_material_required"])
    ...
    st.write("Available Internal Budget:", INTERNAL_APP_BUDGET)
    ...
    rate_per_hour = st.number_input(
        "Final Rate Per Hour",
        min_value=0,
        value=int(selected_request.get("rate_per_hour", 3000))
    )

    stay_cost = st.number_input("Stay Cost", min_value=0, value=0)
    travel_cost = st.number_input("Travel Cost", min_value=0, value=0)
    food_cost = st.number_input("Food Cost", min_value=0, value=0)
    training_material_cost = st.number_input("Training Material Cost", min_value=0, value=0)
    other_cost = st.number_input("Other Expense Cost", min_value=0, value=0)

    hours = int(selected_request["hours"])
    training_days = int(selected_request.get("training_days", 1))

    trainer_cost = rate_per_hour * hours * training_days
    ...
    st.write("Total Estimated Cost:", total_estimated_cost)
    st.write("Internal App Budget:", INTERNAL_APP_BUDGET)

    difference = INTERNAL_APP_BUDGET - total_estimated_cost

    if difference >= 0:
        st.success(f"Within budget. Remaining budget: ₹{difference}")
        budget_status = "Budget Approved"
    else:
        st.error(f"Budget exceeded by ₹{abs(difference)}")
        budget_status = "Budget Problem"

    mediator_remarks = st.text_area("Mediator Remarks")

    decision = st.selectbox(
        "Decision",
        ["Approve and Send to Director", "Send Back to Requester"]
    )
    ...
    if st.button("Submit Decision"):
        ...
        requests_df.loc[index, "training_material_cost"] = training_material_cost
        requests_df.loc[index, "other_cost"] = other_cost
        requests_df.loc[index, "internal_app_budget"] = INTERNAL_APP_BUDGET
        requests_df.loc[index, "budget_status"] = budget_status
        requests_df.loc[index, "total_estimated_cost"] = total_estimated_cost

        requests_df["mediator_remarks"] = requests_df.get(
            "mediator_remarks",
            ""
        ).astype("object")

        requests_df.loc[index, "mediator_remarks"] = str(mediator_remarks)
